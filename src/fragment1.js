...
class Fragment {
  ...
  constructor(selectorInitializer, options) {
    ...
    // TODO Check `selectorInitializer` is supported.
    // We initialize fragment's selector using its BEM base, which must be
    // provided as fragment's class property.
  ...
  transformSelector(transformations, selector, bemBase) {
    ...
    for (const t in requestedTransformations) {

      // 'cns' (component namespace) often used to build one widget on top
      // of other widget.
      if (t === 'cns') {
      ...
      // 'cid' (component id) often used to get specific fragment. It respects
      // 'cns' transformation.
      else if (t === 'cid') {
      ...
      // 'idx' often used to get specific fragment by its index. It respects
      // 'cns' and 'cid' transformations.
      else if (t === 'idx') {
        ...
        if (_.isInteger(v) && v >= 0) {
           ...
           */
          selector = selector.filter((node, idx) => idx === v, { v });
        }
        else {
          throw new TypeError(
