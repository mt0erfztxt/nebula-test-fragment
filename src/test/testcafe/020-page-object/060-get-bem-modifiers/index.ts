...
test("020 fails when 'bemModifierName' argument is not valid", async t => {
  ...
  try {
    ...
    await t
      .expect(e.message)
      .eql("BEM modifier's name does not conform constraints");
  }
  await t.expect(isThrown).ok();
});

test("030 returns promise resolving with array that contains all BEM modifiers when 'bemModifierName' argument not provided", async t => {
  const widgetA1 = new WidgetA();
...
test("040 returns promise resolving with array that contains only BEM modifiers with name equal to 'bemModifierName' argument when it provided", async t => {
  const widgetA1 = new WidgetA();
  const widgetB1 = new WidgetB();
