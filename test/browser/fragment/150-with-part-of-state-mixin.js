...
fixture `Fragment :: 150 .withPartOfStateMixin()`
...
test("010 It should throw error when 'BaseFragment' argument is not a fragment class", async () => {
  ...
  expect(
    ...
    'to throw',
    new TypeError("'BaseFragmentClass' argument must be a 'Fragment' class or its descendant")
  );
});
...
test("020 It should throw error when part of state name is not a non-blank string", async () => {
  expect(
    ...
    'to throw',
    new TypeError("Name of state part must be a non-blank string but it is String (  )")
  );
});
...
test("030 It should throw error when name of attribute/BEM modifier that holds value of part of state is set but it is not a non-blank string", async () => {
  expect(
    ...
    'to throw',
    new TypeError(
      "Attribute/BEM modifier name that holds value of part of state must be a non-blank string but it is String ()"
    )
  );
});
...
test("160 It should generate convenience assertion methods for part of state - case of 'options.waitTil' is true", async () => {
  ...
  // Check that error is thrown when 'options.waitTil' is not a boolean or non-blank string.
  expect(
    () => {
  ...
  // TODO How to test `options.wait`? `t` in test and `t` imported into 'fragment' module isn't the same and because of that we can't spy on `t.wait()`.
...
test("180 It should generate convenience assertion methods for part of state - case of 'options.waitUntil' is true", async () => {
  ...
  // Check that error is thrown when 'options.waitUntil' is not a boolean or non-blank string.
  expect(
    () => {
